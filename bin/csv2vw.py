...
parser.add_argument("-l", "--label_index", help="index of label column (default 0, use -1 if there are no labels)",
                    type=int, default=0)

parser.add_argument("-i", "--ignore_columns",
                    help="index(es) of columns to ignore, for example 3 or 3,4,5 (no spaces in between)")

parser.add_argument("-s", "--skip_headers", help="use this option if there are headers in the file - default false",
                    action="store_true")

parser.add_argument("-z", "--convert_zeros", help="convert labels for binary classification from 0 to -1",
                    action='store_true', default=False)

parser.add_argument("-c", "--print_counter", help="print counter every _ examples (default 10000)", type=int,
                    default=10000)
